...
class CalculadoraTimeOut {
	...
	@Test
	@DisplayName("Probar las aserciones timeOut")
	void testAsercionesAsercionesAgrupadas() {		
	
		Persona personaEsperado = new Persona("Eleazar", "Alfriadez");
		Persona personaActual = new Persona("Eleazarqq", "Alfriadezqq");
			
			assertAll(
					() -> assertEquals(personaEsperado.getApellido(), personaActual.getApellido(), "Primera prueba"),
					() -> assertEquals(personaEsperado.getNombre(), personaActual.getNombre(), "Segunda prueba")
				);
